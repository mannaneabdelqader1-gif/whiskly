...
async function searchByName(reset = true) {
  ...
  if (!grid) return;
  if (!q) {
    grid.innerHTML = `<div class="empty-state"><div class="empty-icon">📖</div><h3>Enter a meal name</h3></div>`;
...
function wireEvents() {
  ...
  if ($("nameShuffleBtn")) $("nameShuffleBtn").onclick = async () => {
    try {
      ...
      if ($("nameResultsTitle")) $("nameResultsTitle").textContent = `${nameResults.length} shuffled meals`;
      if ($("nameMatchInfo")) $("nameMatchInfo").textContent = "Random shuffle from the catalog.";
      showView("search");
    } catch (err) {
      console.error(err);
  ...
  if ($("nameResetBtn")) $("nameResetBtn").onclick = () => {
    ...
    if ($("nameMealGrid")) $("nameMealGrid").innerHTML = `<div class="empty-state"><div class="empty-icon">📖</div><h3>Search by name</h3><p>Type a dish name to explore the catalog.</p></div>`;
    if ($("nameResultsTitle")) $("nameResultsTitle").textContent = "Search by meal name";
    if ($("nameMatchInfo")) $("nameMatchInfo").textContent = "Click load more meals to see additional results.";
    if ($("nameLoadMoreBtn")) $("nameLoadMoreBtn").hidden = true;
  ...
  if ($("nameSearchInput")) $("nameSearchInput").addEventListener("keydown", e => { if (e.key === "Enter") searchByName(); });
  ["nameDietFilter","nameMealType"].forEach(id => { if ($(id)) $(id).addEventListener("change", () => { if ($("nameSearchInput")?.value) searchByName(); }); });
